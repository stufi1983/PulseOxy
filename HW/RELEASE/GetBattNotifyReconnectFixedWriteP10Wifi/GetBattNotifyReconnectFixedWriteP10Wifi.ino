...
String receivedDataS1 = "";  // Variable to hold incoming serial data
bool dataReceivedS1 = false;
...
String storedDevAddr = "00:00:00:00:00:00";
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    if (advertisedDevice.getAddress().toString() == storedDevAddr) {
      deviceFound = true;
      isScanning = false;
...
void connectToDevice() {
  Serial.println("Connecting to BLE device..." + storedDevAddr);
  BLEAddress address(storedDevAddr);
...
void loop() {
  ...
  if (Serial1.available() > 0) {
    //RxData = Serial1.read();
    //Serial.write(RxData);
    char incomingChar1 = Serial1.read();
    if (incomingChar1 == '{') {  // Starting delimiter
      receivedDataS1 = "";       // Reset the data
      dataReceivedS1 = true;
    }

    if (dataReceivedS1) {
      receivedDataS1 += incomingChar1;
    }

    if (incomingChar1 == '}') {  // Ending delimiter
      dataReceivedS1 = false;
      if (receivedDataS1.length() > 2) {                                                // Check if data is valid (excluding delimiters)
        String dataToStore = receivedDataS1.substring(1, receivedDataS1.length() - 1);  // Extract actual data
        Serial.println("Data: " + dataToStore);
        if (storedDevAddr != dataToStore) {
          Serial.println("Save new data: " + dataToStore);
          writeToEEPROM(dataToStore);  // Store data into EEPROM
          storedDevAddr = dataToStore;
          ESP.restart();
        } else {
          Serial.println("Data is already equal");
        }
      }
    }
  }
